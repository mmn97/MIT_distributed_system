...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("First Leader is chosen. \n")
	// if the leader disconnects, a new one should be elected.
	cfg.disconnect(leader1)
	cfg.checkOneLeader()
	DPrintf("Second leader is chosen after the first one is disconnected. \n")
	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
	DPrintf("The come back of first leader does not disturb current situation. \n")
	...
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()
	DPrintf("No leader situation. \n")
	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()
	DPrintf("A leader is chosen. \n")
	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	cfg.checkOneLeader()
	DPrintf("The come back of previous leader does not disturb current situation. \n")
